...
@section('content')
          ...
          <form class="sm:pr-3">
            ...
            <div class="relative mt-1 w-48 sm:w-64 xl:w-96">
              ...
              <datalist id="elementLists">
                @foreach ($basicRequirementLists as $basicRequirement)
                  <option value="{{ $basicRequirement->name }}">{{ $basicRequirement->name }}</option>
                @endforeach
              </datalist>
            </div>
          </form>
          ...
          <table class="min-w-full table-fixed divide-y divide-gray-200 dark:divide-gray-600">
            <thead class="bg-gray-100 dark:bg-gray-700">
              <tr>
                ...
                <th scope="col" class="pe-4 text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
                  Aksi
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
              @foreach ($basicRequirements as $basicRequirement)
                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                  <td class="whitespace-nowrap p-4 text-base font-medium text-gray-900 dark:text-white">
                    {{ $loop->iteration + ($basicRequirements->currentPage() - 1) * $basicRequirements->perPage() }}</td>
                  <td class="whitespace-nowrap p-4 text-base font-medium text-gray-900 dark:text-white">
                    {{ $basicRequirement->name }}</td>
                  {{-- <td class="whitespace-nowrap p-4 text-base font-medium text-gray-900 dark:text-white">
                    {{ $basicRequirement->schemes->map(fn($scheme) => $scheme->code)->join(',') }}
                  </td> --}}
                  <td class="flex justify-center space-x-2 whitespace-nowrap p-4">
                    <a href="{{ route('elements.edit', ['element' => $basicRequirement]) }}"
                    ...
                    <form action="{{ route('elements.destroy', ['element' => $basicRequirement]) }}" method="POST"
  ...
  <div
    ...
    <div class="mb-4 flex items-center sm:mb-0">
      <a href="{{ $basicRequirements->toArray()['first_page_url'] }}"
      ...
      <a href="{{ $basicRequirements->toArray()['last_page_url'] }}"
      ...
      <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Showing <span
          class="font-semibold text-gray-900 dark:text-white">{{ $basicRequirements->firstItem() ?? $basicRequirements->total() }}-{{ $basicRequirements->lastItem() ?? $basicRequirements->total() }}</span>
        of <span class="font-semibold text-gray-900 dark:text-white">{{ $basicRequirements->total() }}</span></span>
    </div>
    <div class="flex items-center space-x-3">
      <a href="{{ $basicRequirements->previousPageUrl() }}"
      ...
      <a href="{{ $basicRequirements->nextPageUrl() }}"
        class="inline-flex flex-1 items-center justify-center rounded-lg bg-emerald-700 px-3 py-2 text-center text-sm font-medium text-white hover:bg-emerald-800 focus:ring-4 focus:ring-emerald-300 dark:bg-emerald-600 dark:hover:bg-emerald-700 dark:focus:ring-emerald-800">
        Next
