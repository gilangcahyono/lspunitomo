  ...
  <form action="{{ route('job-groups.store') }}" method="POST">
    ...
    <div class="mb-5">
      ...
      <select id="scheme_id" name="scheme_id" required
        ...
        <option selected hidden value="{{ request('schemeId') ?? '' }}">
          {{ $schemes->firstWhere('id', request('schemeId'))->name ?? 'Pilih Skema' }}
        </option>
    ...
    <div class="{{ request('schemeId') ? '' : 'hidden' }} mb-5">
...
@push('scripts')
  <script type="module">
    $('#scheme_id').change((e) => {
      e.preventDefault();
      document.location.href = "{{ route('job-groups.create') }}?schemeId=" + $('#scheme_id').val();
      // alert($('#scheme_id').val());
    });
  </script>
@endpush
