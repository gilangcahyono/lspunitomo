      ...
      <div class="block items-center justify-between dark:divide-gray-700 sm:flex md:divide-x md:divide-gray-100">
        <div class="mb-4 flex items-center sm:mb-0">
          <form class="w-full sm:pr-3" method="GET">
            ...
            @if (request('scheme_id') && $jobGroups[0])
              <p id="helper-text-explanation" class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                Ditemukan Skema : {{ $jobGroups[0]->scheme->name }}
              </p>
            @endif
          </form>
        </div>
        <a href="{{ route('job-groups.create') }}"
                ...
                @foreach ($jobGroups as $jobGroup)
                  <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                    <td class="whitespace-nowrap p-4 text-base font-medium text-gray-900 dark:text-white">
                      {{ $loop->iteration + ($jobGroups->currentPage() - 1) * $jobGroups->perPage() }}</td>
                    <td class="whitespace-nowrap p-4 text-base font-medium text-gray-900 dark:text-white">
                      <a class="hover:underline"
                        href="{{ route('job-groups.show', ['job_group' => $jobGroup]) }}">{{ $jobGroup->name }}</a>
                    </td>
                    <td class="whitespace-nowrap p-4 text-base font-medium text-gray-900 dark:text-white">
                      {{ $jobGroup->scheme->name }}</td>
                    <td class="whitespace-nowrap p-4 text-base font-medium text-gray-900 dark:text-white">
                      {{ $jobGroup->units->count() }}</td>
                    <td class="flex space-x-2 whitespace-nowrap p-4">
                      <a href="{{ route('job-groups.edit', ['job_group' => $jobGroup]) }}"
                      ...
                      <button data-modal-target="delete-confirm-modal{{ $jobGroup->id }}"
                        data-modal-toggle="delete-confirm-modal{{ $jobGroup->id }}"
                      ...
                      @include('master.job-group.delete-confirm-modal')
                    </td>
                  </tr>
                @endforeach
              </tbody>
    ...
    <div
      ...
      <div class="mb-4 flex items-center gap-1 sm:mb-0">
        ...
        <select id="show" name="show"
          ...
          <option value="{{ $jobGroups->total() }}" {{ request('show') == $jobGroups->total() ? 'selected' : '' }}>
            All</option>
        </select>
      </div>
      <div class="mb-4 flex items-center sm:mb-0">
        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Showing <span
            class="font-semibold text-gray-900 dark:text-white">{{ $jobGroups->firstItem() ?? $jobGroups->total() }}-{{ $jobGroups->lastItem() ?? $jobGroups->total() }}</span>
          of <span class="font-semibold text-gray-900 dark:text-white">{{ $jobGroups->total() }}</span></span>
      </div>
      <div class="flex items-center space-x-3">
        <a href="{{ $jobGroups->toArray()['first_page_url'] }}"
        ...
        <a href="{{ $jobGroups->previousPageUrl() }}"
        ...
        <a href="{{ $jobGroups->toArray()['last_page_url'] }}"
  ...
  <script type="module">
    $('#show').on('change', (e) => {
      window.location.href =
        `{{ route('job-groups.index') }}?show=${e.target.value}&search=&search={{ request('search') }}&page={{ request('page') }}`;
    });
  </script>
@endpush
    ...
    function tableSchemeRow(schemes) {
      ...
      schemes.forEach((scheme, idx) => {
        htmlView += `<tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
          ...
          <td class="flex space-x-2 whitespace-nowrap p-4">
            <a href="{{ route('schemes.edit', ['scheme' => $jobGroup]) }}"
            ...
            <form action="{{ route('schemes.destroy', ['scheme' => $jobGroup]) }}" method="POST" onclick="return confirm('Are you sure?')">
                @method('DELETE')
                @csrf
                <button type="submit"
                class="inline-flex items-center rounded-lg bg-red-700 px-3 py-2 text-center text-sm font-medium text-white hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
