...
@section('content')
        ...
        {{-- <div class="mb-4 flex items-center sm:mb-0">
          <form class="sm:pr-3">
            ...
            <div class="relative mt-1 w-48 sm:w-64 xl:w-96">
              <input type="text" name="search" id="search" autocomplete="off" list="kukLists"
              ...
              <datalist id="kukLists">
                @foreach ($kukLists as $kuk)
                  <option value="{{ $kuk->name }}">{{ $kuk->name }}</option>
                @endforeach
              </datalist>
            </div>
          </form>
  ...
  @if ($kuks->count() > 0)
                ...
                @foreach ($kuks as $kuk)
                  <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <td class="flex space-x-2 whitespace-nowrap p-4">
                      ...
                      <button data-modal-target="delete-confirm-modal{{ $kuk->id }}"
                        data-modal-toggle="delete-confirm-modal{{ $kuk->id }}"
                      ...
                      @include('master.kuk.delete-confirm-modal')
                    </td>
                  </tr>
                @endforeach
              </tbody>
    ...
    <div
      ...
      <div class="mb-4 flex items-center sm:mb-0">
        {{-- <a href="{{$kuks->toArray()['first_page_url'] }}"
        ...
        {{-- <a href="{{$kuks->toArray()['last_page_url'] }}"
        ...
        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Showing <span
            class="font-semibold text-gray-900 dark:text-white">{{ $kuks->firstItem() ?? $kuks->total() }}-{{ $kuks->lastItem() ?? $kuks->total() }}</span>
  ...
  <script type="module">
    $('#show').on('change', (e) => {
      window.location.href =
        `{{ route('kuks.index') }}?show=${e.target.value}&search=&search={{ request('search') }}&page={{ request('page') }}`;
    });
  </script>
@endpush
