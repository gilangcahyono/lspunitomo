...
@section('content')
  <h1 class="text-center text-xl font-bold text-gray-900 dark:text-white sm:text-xl">
    AK 03 <br> UMPAN BALIK DAN CATATAN ASESMEN <br> <span class="uppercase">SKEMA {{ $scheme->name }}</span>
  </h1>
        ...
        <tr class="rounded-lg border bg-white dark:border-gray-700 dark:bg-gray-800">
          ...
          <td class="border px-6 py-4 dark:border-gray-700">
            <a class="hover:underline"
              href="{{ route('ak-03.accessions', ['schemeId' => $scheme->id]) }}/accessions/{{ $accession->id }}">{{ $accession->nim }}</a>
          </td>
          <td class="border px-6 py-4 dark:border-gray-700">
            {{ $accession->name }}
          </td>
