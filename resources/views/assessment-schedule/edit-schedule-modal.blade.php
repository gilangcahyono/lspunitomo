<div id="edit-schedule-modal-{{ $schedule->id }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
      <div class="flex items-center justify-between rounded-t border-b p-4 dark:border-gray-600 md:p-5">
        ...
        <button type="button"
          ...
          data-modal-hide="edit-schedule-modal-{{ $schedule->id }}">
      ...
      <div class="p-4 md:p-5">
        <form class="space-y-4" method="POST"
          action="{{ route('assessment-schedules.update', ['assessment_schedule' => $schedule]) }}">
          @method('PUT')
          @csrf
          <div class="flex gap-2">
            <input type="datetime-local" name="schedule" value="{{ $schedule->schedule }}"
              ...
              required />
            <input type="text" name="tuk" value="{{ $schedule->tuk }}"
              class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-500 dark:bg-gray-600 dark:text-white dark:placeholder-gray-400"
              required />
          </div>
          <button type="submit"
